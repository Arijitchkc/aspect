...
double Katz2003MantleMelting<dim>::reference_darcy_coefficient() const {
  // 0.01 = 1% melt
  return reference_permeability * Utilities::fixed_power<3>(0.01) /
         viscosity_fluid;
}

template <int dim>
double Katz2003MantleMelting<dim>::melt_fraction(const double temperature,
...
template <int dim>
double Katz2003MantleMelting<dim>::entropy_change(
  ...
  // we need the change of melt fraction in dependence of pressure and
  // temperature
  ...
  if (temperature > T_solidus && temperature < T_liquidus &&
    ...
    double melt_fraction_derivative_temperature =
        beta *
        std::pow((temperature - T_solidus) / (T_lherz_liquidus - T_solidus),
                 beta - 1) /
    ...
    if (melt_fraction(temperature, pressure) > F_max) {
      const double T_max =
          std::pow(F_max, 1.0 / beta) * (T_lherz_liquidus - T_solidus) +
          T_solidus;
      const double dF_max_dp =
          -M_cpx * Utilities::fixed_power<-2>(r1 + r2 * pressure) * r2;
      const double dT_max_dp =
          dT_solidus_dp +
          1.0 / beta * std::pow(F_max, 1.0 / beta - 1.0) * dF_max_dp *
              (T_lherz_liquidus - T_solidus) +
          std::pow(F_max, 1.0 / beta) * (dT_lherz_liquidus_dp - dT_solidus_dp);

      melt_fraction_derivative_temperature =
          (1.0 - F_max) * beta *
          std::pow((temperature - T_max) / (T_liquidus - T_max), beta - 1) /
          (T_liquidus - T_max);
      ...
      melt_fraction_derivative_pressure =
          ...
          (1.0 - F_max) * beta *
              std::pow((temperature - T_max) / (T_liquidus - T_max), beta - 1) *
              (dT_max_dp * (T_max - T_liquidus) -
               (dT_liquidus_dp - dT_max_dp) * (temperature - T_max)) /
    ...
    if (melt_fraction(temperature, pressure) >= maximum_melt_fraction)
      entropy_gradient =
          melt_fraction_derivative * peridotite_melting_entropy_change;
  }
  ...
  for (unsigned int i = 0; i < in.n_evaluation_points(); ++i) {
    ...
    if (this->include_melt_transport()) {
      ...
      if (fractional_melting) {
        ...
        const double solidus_change =
            (maximum_melt_fraction - old_porosity) * depletion_solidus_change;
        const double eq_melt_fraction = melt_fraction(
            in.temperature[i] - solidus_change,
        ...
        // If the porosity is larger than the equilibrium melt fraction, melt
        // should freeze again. Because we do not track the melt composition, we
        // have to use a workaround here for freezing of melt: We reduce the
        // porosity until either it reaches the equilibrium melt fraction, or
        // the depletion (peridotite field), which decreases as melt freezes,
        // reaches the same value as the equilibrium melt fraction, whatever
        // happens earlier. An exception is when the melt fraction is zero; in
        // this case all melt should freeze.
        const double eq_melt_fraction = melt_fraction(
            in.temperature[i],
            this->get_adiabatic_conditions().pressure(in.position[i]));

        // If the porosity change is not negative, there is no freezing, and the
        // change in porosity is covered by the melting relation above.
        ...
        // We have to multiply with the melting time scale here to obtain the
        // porosity change that happens in the time defined by the melting time
        // scale (as opposed to a rate). This is important because we want to
        // perform some checks on this quantity (for example, we want to make
        // sure that this change does not lead to a negative porosity, see
        // below). Later on, the overall porosity change is then divided again
        // by the melting time scale to obtain the rate of melting or freezing,
        // which is used in the operator splitting scheme.
      ...
      if (this->get_geometry_model().depth(in.position[i]) < extraction_depth)
        porosity_change =
            -old_porosity *
            (in.position[i](1) -
             (this->get_geometry_model().maximal_depth() - extraction_depth)) /
            extraction_depth;
...
template <int dim>
void Katz2003MantleMelting<dim>::calculate_fluid_outputs(
    const typename Interface<dim>::MaterialModelInputs &in,
...
template <int dim>
void Katz2003MantleMelting<dim>::declare_parameters(ParameterHandler &prm) {
  prm.declare_entry("A1", "1085.7", Patterns::Double(),
  ...
  prm.declare_entry("r2", "8e-11", Patterns::Double(),
                    "Prefactor of the linear pressure term "
  ...
  prm.declare_entry("beta", "1.5", Patterns::Double(),
                    ...
                    "Units: non-dimensional.");
  prm.declare_entry("Mass fraction cpx", "0.15", Patterns::Double(),
...
template <int dim>
void Katz2003MantleMelting<dim>::parse_parameters(ParameterHandler &prm) {
